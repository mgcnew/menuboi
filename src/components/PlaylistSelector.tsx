...
export const PlaylistSelector = ({ onSelect, onStart }: PlaylistSelectorProps) => {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      const totalOptions = playlists.length + 1; // +1 for "All tracks" option
      ...
      switch (e.key) {
        ...
        case " ":
          e.preventDefault();
          if (selectedIndex === 0) {
            onSelect(null); // All tracks
          } else {
            onSelect(playlists[selectedIndex - 1].id);
          }
          onStart();
          break;
      }
  ...
  }, [playlists, selectedIndex, onSelect, onStart]);

  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-white text-2xl">Carregando playlists...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-8">
      ...
      {playlists.length === 0 && (
        <p className="text-slate-500 mt-8 text-lg">
          Nenhuma playlist criada. Crie playlists no painel admin.
        </p>
      )}
    </div>
  );
